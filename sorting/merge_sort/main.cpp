...
#include "data_set.cpp";
  ...
  std::cout<<"\n\n\n=====RESULT=====\n\n\n";
  std::for_each(arr, arr+arr_size, [](int el){std::cout<<el<<" ";});
  return 0;
}
...
void mergeResults(T* array_first,
  ...
  T * result_array = new T [result_size];
  ...
  std::move(result_array, result_array+result_size, array_first);
  delete[] result_array;
}
